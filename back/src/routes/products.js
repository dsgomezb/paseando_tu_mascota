...
router.get('/edit/:id', isLoggedIn, async (req, res) => {
    if(req.user.id_profile == 2){
        ...
        const category_product = await pool.query('SELECT * FROM category_product WHERE status_category_product = 3 AND id_category_product != ? ORDER BY name_category_product desc', [product.id_category_product]);
        const presentation_product = await pool.query('SELECT * FROM presentation_product WHERE status_presentation_product = 3 AND id_presentation_product != ? ORDER BY name_presentation_product', [product.id_presentation_product]);
        const establecimientos = await pool.query('SELECT * FROM establecimiento AS est INNER JOIN establecimiento_user AS est_usr ON est_usr.id_establecimiento = est.id_establecimiento INNER JOIN users AS usr ON usr.id_user = est_usr.id_user INNER JOIN municipios AS mun ON mun.id_muni = est.id_muni WHERE est_usr.id_user = ? AND est.id_establecimiento != ? ORDER BY est.created_at desc', [id_user, product.id_establecimiento]);
        res.render('products/edit', {product, category_product, presentation_product, establecimientos});
    }else{
        res.status(401);
        res.render('partials/errors/route');
    }
...
//Ruta para actualizar un nuevo producto
router.post('/update', async (req, res) => {
    if(req.user.id_profile == 2){
        let data = req.body;
        let created_at = new Date();
        const { name_product, id_establecimiento, description_product, internal_code_product, id_category_product, id_presentation_product, unitary_value_product, iva_product, status_product, id_product} = data;
        let value_iva_product = (unitary_value_product*iva_product)/100; 
        value_iva_product = parseFloat(unitary_value_product)+parseFloat(value_iva_product);
        const newProduct = {
            name_product,
        ...
        const name_product_result = await pool.query('SELECT name_product FROM product WHERE name_product = ? AND id_establecimiento = ? AND id_product != ?', [newProduct.name_product, newProduct.id_establecimiento, id_product]);
        const internal_code_product_result = await pool.query('SELECT internal_code_product FROM product WHERE internal_code_product = ? AND id_establecimiento = ? AND id_product != ?', [newProduct.internal_code_product, newProduct.id_establecimiento, id_product]);
        const image_product_result = await pool.query('SELECT image_url_product FROM product WHERE id_product = ?', [id_product]);
        ...
        if(name_product_result.length > 0){
            ...
            let image_url_product = null;
            let image_name_bd = image_product_result[0].image_url_product;
            if(req.files != null){
            ...
            newProduct.image_url_product = image_name_bd;
            const query_presentation_product = await pool.query('UPDATE product SET ? WHERE id_product = ?', [newProduct, id_product]);
            if(query_presentation_product){
                return res.send(true);
            }else{
                return res.send(false);
